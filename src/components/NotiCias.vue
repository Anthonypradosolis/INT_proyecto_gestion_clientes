    ...
    <form @submit.prevent="agregarNoticia" class="mb-4">
      <div class="mb-3 row align-items-center">
        ...
        <div class="col-md-4 ms-auto d-flex align-items-center justify-content-end">
          <small class="text-muted">Noticias guardadas: {{ noticias.length }}</small>
        </div>
      </div>
      ...
      <table class="table noticias-table mt-3">
        ...
        <tbody>
          <tr v-for="noticia in noticias" :key="noticia.id">
            <td class="align-middle"><strong class="text-primary">{{ noticia.titulo }}</strong></td>
            <td class="align-middle">
              <div class="contenido">
                <span v-if="isExpanded[noticia.id]">{{ noticia.contenido }}</span>
                <span v-else>{{ noticia.contenido.length > 200 ? noticia.contenido.slice(0,200) + '...' : noticia.contenido }}</span>
                <a href="#" @click.prevent="toggleExpand(noticia.id)" class="ms-2 text-decoration-none small">
                  {{ isExpanded[noticia.id] ? 'Mostrar menos' : 'Seguir leyendo' }}
                </a>
              </div>
            </td>
            <td class="align-middle"><small class="text-muted">{{ noticia.fecha }}</small></td>
          ...
          <tr v-if="noticias.length === 0"><td colspan="4" class="text-center text-muted">No hay noticias aún.</td></tr>
        </tbody>
      </table>
    </div>
  </div>
...
const agregarNoticia = async () => {
  ...
  const payload = { titulo: nuevo.titulo.trim(), contenido: nuevo.contenido.trim(), fecha: new Date().toLocaleDateString() }
...
const eliminarNoticia = async (id) => {
  ...
  }catch(e){ console.error('error eliminando noticia', e) }
}
